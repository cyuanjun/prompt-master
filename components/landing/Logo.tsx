...
export function Logo({ size = "md" }: LogoProps) {
  const isSm = size === "sm";
  const fontSize = isSm ? "1.5rem" : "clamp(1.75rem, 8vw, 4rem)";
  ...
  return (
    <div className="relative inline-block text-center leading-[0.95]">
      <span
        className="font-logo logo-text block text-ink"
        style={{ fontSize }}
      >
        Prompt
      </span>
      <span
        className="font-logo logo-text block"
        style={{
          fontSize,
          color: "var(--purple-dark)",
        }}
      >
        Battle <span className="text-ink">Arena</span>
      </span>
      <svg
        aria-hidden="true"
        viewBox="0 0 64 48"
